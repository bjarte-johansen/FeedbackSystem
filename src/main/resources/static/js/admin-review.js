...
var AdminReview = {
    ...
    triggerClientReviewStatusFilterChange(select) {
        this.reloadReviewList();
    },
    ...
    buildSearchParamsFromReviewList($reviewList) {
        ...
        const statusFilter = $reviewList.attr("data-status-filter");
    ...
    updateReviewListFromUI() {
        ...
        const $reviewStatusFilter = $("select[name='reviewStatusFilter']");
        if($reviewStatusFilter.length !== 0) {
            const reviewStatusFilterValue = $reviewStatusFilter.val();
            $reviewList.attr("data-status-filter", reviewStatusFilterValue);
        }
    },

    reloadReviewList: function () {
        AdminReview.updateReviewListFromUI();

        const $reviewList = $(".review--list");
        if (!$reviewList.length) return console.warn("No .review--list element found for reloadReviewList");

        const params = AdminReview.buildSearchParamsFromReviewList($reviewList);
    ...
    prevReviewListPage: function () {
        AdminReview.nextReviewListPage(-1);
    },
    ...
    handlers: {
        ...
        likeReviewDone: function (form, res) {
            ...
            AdminReview.reloadReview(reviewId);
        }, dislikeReviewDone: function (form, res) {
            console.log("AdminReview.formHandlers.dislikeReviewDone called");
            if (res.status !== 200) return;
        ...
        deleteReviewDone: function (form, res) {
            console.log("AdminReview.formHandlers.deleteReviewDone called");
        }, submitReviewDone: function (form, res) {
            console.log("AdminReview.formHandlers.submitReviewDone called");
        },
        ...
        markApprovedReviewDone: function (form, res) {
            ...
            console.log("AdminReview.formHandlers.markApprovedReviewDone called");
        }, markRejectedReviewDone: function (form, res) {
            if (res.status !== 200) return;

            console.log("AdminReview.formHandlers.markRejectedReviewDone called");
        }, markPendingReviewDone: function (form, res) {
            if (res.status !== 200) return;

            console.log("AdminReview.formHandlers.markPendingReviewDone called");
        }
    },
...
document.addEventListener("DOMContentLoaded", function () {
    document.addEventListener("submit", async e => {
        ...
        if (form.dataset.handler) {
            const fn = AdminReview.handlers[form.dataset.handler];
            if (fn) fn(form, res);
        }
    });

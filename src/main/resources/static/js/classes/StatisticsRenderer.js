class StatisticsRenderer {
    static updateFromJson(stats) {
        ...
        $reviewStats.find(".big-average-score")
            .text(bigAverageScore + " / 5");

        // update score (stars)
        $reviewStats.find(".score")
            .addClass("score-" + bigAverageScore.replace(".", "-"));

        // update average score
        $reviewStats.find(".average-score")
            .text(bigAverageScore);

        // update total review count
        $reviewStats.find(".total-review-count")

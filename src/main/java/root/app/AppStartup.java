...
class AppStartup implements ApplicationRunner {
    ...
    public void doDatabaseInitSlowForTesting(){
        ...
        Logger.log("AppStartup::run, SCHEMA: " + AppRequestSchema.get());

        // reset demo data
        QueryLogger.start();

        FSQLQuery.setQueryLogger(QueryLogger.getSingleton());
        try(var __ = new TryWithTimer("Old slow insertion with single SQL statement")) {
    ...
    public void doDatabaseInitFastForTesting(){
        ...
        INSERT INTO Tenant(name, domain, api_key, email, password_hash, password_salt, schema_name)
        VALUES ('Tenant1', 'tenant1.test.com', 'tenant-1-api-key', 'tenant1@test.com', '$2a$10$02gG0fQltKtHCB950h3q5.jL9z4mEh/cdoHmOIaKtw2PD0s1H5F7i', '', 'test');
        ...
        AppRequestSchema.withThreadSchema("public", fn);
    }
}

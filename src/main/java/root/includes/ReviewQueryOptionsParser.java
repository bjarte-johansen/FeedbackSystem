...
public class ReviewQueryOptionsParser {
    private final static boolean LOG_PARSED_OPTIONS = false;
    ...
    private static Set<Integer> parseIntSetOrEmptySet(String v) {
        return new HashSet<>( parseIntListOrEmptyList(v) );
    }
    ...
    public static ReviewQueryOptions parseRequest(
        ...
        ImmutableUnboundedDateRange<LocalDate> dateRange = new ImmutableUnboundedDateRange<>(null, null);
        dateRange = dateRange.withStart( parseDateOrDefault(req.getParameter("startDateFilter"), null) );
        dateRange = dateRange.withEnd( parseDateOrDefault(req.getParameter("endDateFilter"), null) );
        options.setDateFilterRange(dateRange);
